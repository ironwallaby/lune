...
function phase (phase_date) {
  ...
  const t = (julian.fromDate(phase_date) - 2451545) * (1 / 36525)
  ...
  const sin_d = dsin(d)
  const cos_d = dcos(d)
  const sin_2d = 2 * sin_d * cos_d
  const cos_2d = 2 * cos_d * cos_d - 1

  // solar mean anomaly (p. 338)
  const m = 357.5291092 + t * (35999.0502909 - t * (0.0001536 + t * (1 / 24490000)))
  const sin_m = dsin(m)
  const cos_m = dcos(m)
  const cos_2m = 2 * cos_m * cos_m - 1

  // lunar mean anomaly (p. 338)
  const n = 134.9633964 + t * (477198.8675055 + t * (0.0087414 + t * ((1 / 69699) - t * (1 / 14712000))))
  const sin_n = dsin(n)
  const cos_n = dcos(n)
  const sin_2n = 2 * sin_n * cos_n
  const cos_2n = 2 * cos_n * cos_n - 1
  const sin_2d_n = sin_2d * cos_n - cos_2d * sin_n
  const cos_2d_n = cos_2d * cos_n + sin_2d * sin_n

  // lunar phase angle (p. 346)
  const i = 180 - d - 6.289 * sin_n + 2.100 * sin_m - 1.274 * sin_2d_n - 0.658 * sin_2d - 0.214 * sin_2n - 0.110 * sin_d
  const cos_i = dcos(i)

  // fractional illumination (p. 345)
  const illumination = cos_i * 0.5 + 0.5
  ...
  // XXX: the book is not clear on how many terms to use! I used all the easy
  // ones that we already have for free above, but I imagine this is more than
  // necessary...
  const moon_distance = 385000.56 - 20905.355 * cos_n - 3699.111 * cos_2d_n - 2955.968 * cos_2d - 569.925 * cos_2n + 108.743 * cos_d

  // solar distance
  // https://aa.usno.navy.mil/faq/docs/SunApprox.php
  const sun_distance = tokm(1.00014 - 0.01671 * cos_m - 0.00014 * cos_2m)
  ...
  return {
    ...
    sun_angular_diameter: SUN_ANGULAR_SIZE_SMAXIS / sun_distance
  }
}
