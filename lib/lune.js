 ...
 * @param  {Date} phaseDate Date to get phase information of.
 * @return {Object}          Phase data
 */
function phase (phaseDate) {
  if (!phaseDate) {
    phaseDate = new Date()
  }

  const t = (julian.fromDate(phaseDate) - 2451545) * (1 / 36525)
  ...
  const cos2n = 2 * cosn * cosn - 1
  const sin2dn = sin2d * cosn - cos2d * sinn
  const cos2dn = cos2d * cosn + sin2d * sinn
